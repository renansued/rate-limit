...
public class MockDistributedKeyValueStore implements DistributedKeyValueStore {
    ...
    public void setFailNextN(int n) { this.failNextN = n; }
    ...
    public CompletableFuture<Integer> incrementByAndExpire(String key, int delta, int expirationSeconds) throws Exception {
        ...
        if (failNextN > 0) {
            failNextN--;
            CompletableFuture.delayedExecutor(5, TimeUnit.MILLISECONDS).execute(() -> fut.completeExceptionally(new RuntimeException("simulated transient failure")));
            return fut;
        }
    ...
    public int get(String key) { AtomicInteger ai = map.get(key); return ai == null ? 0 : ai.get(); }
